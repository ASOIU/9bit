---
---
//NOTE: to use outside of Jekyll - remove two lines of "---" (above) and all "{{ site.baseurl }}" (below)
		...
		return {
			btnClick: function(){ //attach handler by using 'querySelectorAll(".crdon-here > .crdon_holder > button")' for safe use '.parentNode' in handler
				...
				if(this !== _mpoControlButton){
					...
					_T_mpo_qr_code_.src = "{{ site.baseurl }}/theme/crypto-currency/qr/" + btn.crdonWalletAddress + ".png"; // or "https://blockchain.info/qr?data="+btn.crdonWalletAddress+"&size=111"
	...
	return {
		init: function(){ // init + garbage collection
			...
			for(i=places.length;i--;){
				...
				currBtn.querySelectorAll("button > img")[0].src = "{{ site.baseurl }}/theme/crypto-currency/icons_cc/-" + uri.scheme + ".png";
